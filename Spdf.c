...
#define PORT 9224  // Define the port number for the Spdf server
#define BUFFER_SIZE 1024  // Define the buffer size for reading and writing data
#define BASE_SAVE_DIR "/home/shah5w1/dfs/spdf/"  // Base directory for saving .pdf files on the server
...
// Function to create a tar file for .pdf files
void create_tar_file(const char *tar_filename) {
    char command[BUFFER_SIZE];
    // Construct the command to find all .pdf files and archive them into a tar file
    snprintf(command, sizeof(command), "find %s -type f -name '*.pdf' | tar -cvf %s -T -", BASE_SAVE_DIR, tar_filename);
...
// Function to list all .pdf files and send the list to Smain
void list_files(int new_socket) {
    char command[BUFFER_SIZE];
    char file_list[BUFFER_SIZE] = "";

    // Construct the command to find all .pdf files in the base directory
    snprintf(command, sizeof(command), "find %s -type f -name '*.pdf'", BASE_SAVE_DIR);
...
int main() {
    ...
    while (1) {
        ...
        if (strcmp(command, "ufile") == 0) {
            ...
            // Create and send the tar file for .pdf files
            create_tar_file(filename);
            send_tar_file(new_socket, filename);
        } else if (strcmp(command, "list") == 0) {
            // List all .pdf files and send the list to Smain
